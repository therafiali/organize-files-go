...
func organizeFolder(targetFolder string) {
	...
	// read the dir
	filesAndFolder, err := os.ReadDir(targetFolder)
	...
	for _, filefilesAndFolder := range filesAndFolder {

		// check for files
		if !filefilesAndFolder.IsDir() {
			fileInfo, err := filefilesAndFolder.Info()
			check(err)
			...
			case ".mp4", ".mov", ".avi", ".amv":
				newPath := filepath.Join(targetFolder, "Vidoes", fileInfo.Name())
	...
	println("Files Organized", noOfFiles)
}
